...
extension Router {
  ...
  private func boolBinding<T>(keyPath: WritableKeyPath<State, T?>) -> Binding<Bool> {
    Binding(
      get: {
...
extension Router {
  ...
  fileprivate func binding<T>(keyPath: WritableKeyPath<State, T>) -> Binding<T> {
    Binding(
      get: {
